\chapter{Geração de um PRNG mediante mapa caótico}\label{cap:3}


{\lettrine[loversize=0.25,findent=0.2em,nindent=0em]{N}{este} capítulo propomos técnicas para o projeto de PRNG a partir de mapas caóticos e introduzimos técnicas de discretização codificada 
variante no tempo. Apesar dos métodos considerados serem independentes do mapa caótico, emprega-se para o estudo de caos o mapa tanh. Para testar a aleatoriedade das sequências geradas empregamos 
...
binária \cite{beiram13}. Seja $\{ Z_n \}_{n=1}^{\infty}$ uma sequência de variáveis aleatórias binárias  estacionárias. 
...
\] 
A entropia condicional $H(Z_n \mid Z^{n-1})$ é uma função não crescente com $n$ para um processo estacionário e converge
para a taxa de entropia $H$~\cite{cover06}
\[
H = \lim_{n \rightarrow \infty} H(Z_n \mid Z^{n-1}). 
\]
Uma sequência aleatória ideal deve atingir  $H(Z_n \mid Z^{n-1})=1$ para todos os valores de $n$. Para gerar uma sequência binária $\{Z_n\}$ a partir da família de mapas tanh, utiliza-se o 
procedimento a seguir. Fixamos um valor de $r$ e uma condição inicial $x_0$ e obtemos uma órbita finita usando $x_n= f(x_{n - 1}), n=1,2,...$. Os primeiros 400 pontos gerados são descartados 
...
As Figuras \ref{fig:entropiar} e \ref{fig:entropiai} mostram $H(Z_n \mid Z^{n-1})$ versus $n$ para uma sequência binária gerada por simulação
...
e após este valor converge para a taxa de entropia. Denomina-se de $n^{*}$ o valor limite de $n$ a partir do qual a entropia condicional permanece constante, e igual a $H$. Por exemplo, 
observa-se na Figura \ref{fig:entropiar} que $n^{*} = 7$.
...
\begin{figure}[!t]
 ...
 \end{pspicture}
 \caption{\label{fig:entropiar} $H(Z_n \mid Z^{n-1})$ versus $n$ para mapa e-tanh com $q=1$ e $r=3,4,5$.}
\end{figure}
...
\begin{figure}[!t]
 ...
 \rput{90}(-.5,2.1){\small $H(Z_n/Z^{n-1})$}
 \end{pspicture}
 \caption{\label{fig:entropiai} $H(Z_n \mid Z^{n-1})$ versus $n$ para mapa o-tanh com $q=1$ e $r=3,4,5$.}
\end{figure}
...
Observe-se nas curvas da Figura~\ref{fig:entropiar} que o valor da taxa de entropia é $H=0,867$ para $r=3$, $H=0,94$ para $r=4$ e $H=0,917$ para $r=5$. 
A incerteza sobre uma variável $Z_n$ decresce com o conhecimento de valores passados até $n=7$, 
o que implica a presença de uma memória finita na sequência binária. Não se espera uma correlação relevante 
entre as variáveis $Z_n$ e $Z_{n+m}$ para $m > 7$, para todos os valores de $r$ considerados. 

As curvas da Figura~\ref{fig:entropiai} mostram que a entropia condicional do mapa o-tanh decresce até $n^{*} = 5$ para $r=3$ e $4$, e para $r=5$ o valor de $n^*$ é $6$. Os valores da 
taxa de entropia neste caso são $H=0,937$ para $r=3$, $H=0,971$ para $r=4$ e $H=0,950$ para $r=5$.
...
A próxima seção descreve o procedimento de geração de um PRNG proposto nesta dissertação a partir da família de mapas tanh. 

\section{Geração de um PRNG baseado na família mapa tanh} 

A geração de um PRNG é composta de três etapas: geração de uma órbita finita a partir de um mapa
caótico, emprego da técnica de saltos de amostras~\cite{micco.08} para reduzir a correlação da sequência caótica
e, finalmente, a sequência binária é gerada usando a técnica de discretização codificada (fixa no tempo ou variante no tempo). Tal processo é ilustrado no diagrama em blocos da Figura ~\ref{fig:blocos1}.
\begin{figure}[b!]
 \begin{pspicture}(-.6,0.5)(3.5,3)
 \psframe(1,1)(4,2)
 ...
 \rput{0}(8.6,1.7){\small $X^{\prime}_k$}
 \rput{0}(12.9,1.7){\small $Z_{k1} ... Z_{kq}$}
 \end{pspicture}
 \caption{\label{fig:blocos1} Diagrama em bloco do esquema da geração de um PRNG.}
\end{figure}
...
saída $\{x^{\prime}_{k}\}_{k=1}^{N^\prime} = \{x_{kp}\}_{k=1}^{N^\prime}$, para um valor fixo do parâmetro $p$ que especifica o salto de amostras. 
...
a sequência binária também tem comprimento $p$ vezes menor que a sequência caótica de entrada, o que compromete a taxa de \textit{bits} por amostra caótica gerada na saída do codificador.

Para aumentar a taxa de \textit{bits}, propõe-se particionar o intervalo $[-1,1]$ em $2^q$ regiões, $\mathcal{R}_i$, $i=0,\cdots {2^q}-1$,   
...
Esta partição reflete a concentração do histograma em valores extremos do mapa, como mostrado nas Figuras \ref{fig:Fig18} e \ref{fig:Fig19}. O mapa o-tanh mostra que as
duas regiões dos extremos e as duas regiões internas tem a mesma largura. Enquanto o mapa e-tanh não apresenta o mesmo comportamento, devido à acumulação de mais pontos em uma extremidade que na 
...
taxa de \textit{bits} por amostra caótica, denotada por $R_x$, da seguinte forma:

\begin{equation}
R_x= q/p.
\end{equation}

Por exemplo, para $R_x =1$ a sequência binária e a sequência caótica têm o mesmo comprimento, ou seja, gera-se a mesma quantidade de \textit{bits} por amostras que os saltos dados. 
Por outro lado, o particionamento em duas regiões implica que $R_x=1/p$. 

O processo de codificação consiste em atribuir a cada região uma sequência de \textit{bits} (denominada de sequência código). Por exemplo, uma codificação fixa no tempo das regiões, denominada de CFT,  
...
Este comportamento é observado nas curvas de entropia condicional mostradas nas Figuras~\ref{fig:graf1} e ~\ref{fig:graf2}. Nestas curvas, fixa-se os parâmetros $r =3$ e $p=1$ (sem saltos 
de amostras), e considera-se três valores de $q$. Observa-se que a taxa de entropia decresce e o valor de $n^{*}$ cresce com o aumento de $q$ para os mapas e-tanh (Figura~\ref{fig:graf1}) 
...
Na Figuras ~\ref{fig:graf1} e Figura ~\ref{fig:graf2} observa-se o comportamento esperado, a codificação CFT introduz uma correlação a mais na sequência binária de saída. 
Para diminuir o efeito desta correlação propõe-se uma codificação variante no tempo , denominada CVT, que consiste em um deslocamento cíclico de $q$ \textit{bits} para 
esquerda entre os \textit{bits} que rotulam regiões adjacentes. Por exemplo, 
se para a $n$-ésima amostra caótica a codificação é $(00, 01, 10, 11)$, para a próxima amostra a codificação passa a ser $(01, 10, 11, 00)$. As curvas de entropia condicional utilizando a CVT 
são mostradas nas Figuras~\ref{fig:CVTpar} e \ref{fig:CVTimpar}. Utilizam-se os mesmos valores de $r$ das Figuras~\ref{fig:entropiar} e ~\ref{fig:entropiai}, e parâmetros de $p=1$ e $q=1$.
...
\begin{figure}[!t]
 ...
 \rput{90}(-.5,2.1){\small $H(Z_n/Z^{n-1})$}
 \end{pspicture}
 \caption{\label{fig:CVTpar} $H(Z_n \mid Z^{n-1})$ versus $n$ para mapa e-tanh para $q=1$, $r=3,4,5$, com CVT.}
\end{figure}
...
\begin{figure}[!t]
 ...
 \rput{90}(-.5,2.1){\small $H(Z_n/Z^{n-1})$}
 \end{pspicture}
 \caption{\label{fig:CVTimpar} $H(Z_n \mid Z^{n-1})$ versus $n$ para mapa o-tanh para $q=1$, $r=3,4,5$, com CVT.}
\end{figure}
...
Esta codificação apresenta um aumento de taxa de entropia. O valor de $n^{*}$ para o mapa e-tanh na Figura~\ref{fig:CVTpar} é igual a 6, e na Figura~\ref{fig:CVTimpar} note-se que $n^{*}=4$ para 
$r=3$, e $n^{*}=5$ para $r=4$ e $5$. Por exemplo, comparando-se as Figuras~\ref{fig:entropiai} e \ref{fig:CVTimpar} para $r=3$, pode-se observar que o valor de $n^{*}$ é menor utilizando a 
codificação CVT. Na Tabela~\ref{tab:n} se faz um resumo dos valores de $n^{*}$ para os diferentes valores de $r$ utilizados. Também obtemos uma taxa de entropia melhor para CVT ($H = 0,9423$) 
que para CFT ($H = 0,8756$) para o valor de $r=3$. 
...
\begin{table}[]
\centering
\caption{Valores de $n^{*}$ para os mapas e-tanh e o-tanh com $r = 3, 4, 5$, $q=1$ e $p=1$.}
\label{tab:n}
...
Para cada valor de $r$, discutiremos valores apropriados de $p$ e $q$, que quando associados às técnicas CFT e CVT produzem PRNG com maior taxa $R_x$.
 
\section{Teste de aleatoriedade NIST}

Para concluir se uma estratégia de geração de um PRNG é criptograficamente segura, as sequências geradas devem ser submetidas a uma variedade de testes estatísticos. O objetivo é detectar 
características específicas esperadas de sequências aleatórias. Existem várias opções disponíveis de testes estatísticos, entre as quais destacam-se: 
NIST~\cite{nist}, DIEHARD~\cite{56}, testes de  Knuth~\cite{knuth.81}. 

Neste trabalho empregara-se o NIST (versão 800-22) que é um pacote que compreende 15 testes estatísticos baseados em testes de hipóteses, e é largamente usado na literatura. No Apêndice A, apresenta-se
...
Cada teste é utilizado para determinar a aceitação ou a rejeição da hipótese que detecta um desvio da aleatoriedade ideal e gera um parâmetro $P$-$value$ que indica a probabilidade 
...
no intervalo [0,001 ; 0,01]. Se $P$-$value$ é igual a 1, então a sequência parece ter aleatoriedade perfeita, enquanto $P$-$value$ igual a 0, então indica que a sequência parece ser completamente 
...
$\alpha$. Para os valores considerados, a razão mínima usada é igual 0,9767. Portanto, se $P$-$value$ é maior que $\alpha$ e a razão é maior que  0,9767 então a sequência é considerada 
...
taxa em relação à mostrada na mesma coluna para um valor menor de $p$. Dado um valor de $p$ o emprego de CVT permite aumentar substancialmente a taxa, como por exemplo é observado 
na Tabela \ref{tab:taxas}. 
Para $r=3$ e $p=7$ no mapa e-tanh, obtemos uma taxa de \textit{bits} $R_x=1/7$ utilizando a codificação CFT. Utilizando a codificação variante no tempo obtém-se uma taxa $R_x=7/7$.
O gerador não passa no teste para $p=7$ e codificação fixa quando utilizamos o parâmetro $r=4$ e $r=5$. O aumento do salto 
para $p=8$ ou $p=9$ ainda permite obter um bom gerador com codificação fixa para $q>1$, apesar de uma baixa taxa de \textit{bits}. Observa-se que é possível obter um gerador com taxa menor que 
1 que passe nos testes com $p=6$ e CVT. O esquema CVT atinge a maior taxa (unitária) com $p=7$ para os valores de $r=3$ e $r=4$, para $r=5$ não se obtém a taxa de \textit{bits} unitária. 
O aumento da taxa propiciado pela codificação CVT também é observado na Tabela~\ref{tab:taxa}
...
Como se pode observar, o mapa o-tanh perde a correlação em valores de $n^{*}$ menores que o mapa e-tanh. A taxa $R_x$ unitária é atingida com um valor de saltos menor para o mapa o-tanh ($p=4$) 
que para o mapa e-tanh ($p=7$) para o mesmo valor de $r=3$ e utilizando a CVT. A Tabela \ref{tab:entc} ilustra que a taxa de entropia deve ter um valor elevado para que a sequência binária
passe em todos os testes NIST. Convém ressaltar que a taxa de entropia para estas sequências é maior que $H=0,9999$ para todos os casos sendo um indicador adicional da aleatoriedade 
...
\multirow{3}{*}{\begin{tabular}[c]{@{}c@{}}Mapa e-tanh\\ CFT\end{tabular}} & $q = 1$  $p = 5$                                                  & $H$= 0,9997                                                       & Não Sucesso \\ \cline{2-4} 
                                                                           ...
                                                                           & \textbf{$q = 1$  $p = 7$}                                                  & \textbf{$H$= 0,9999}                                                       & Sucesso     \\ \hline
\multirow{3}{*}{\begin{tabular}[c]{@{}c@{}}Mapa o-tanh\\ CFT\end{tabular}} & $q = 1$  $p = 3$                                                  & $H$= 0,9990                                                       & Não Sucesso \\ \cline{2-4} 
                                                                           ...
                                                                           &  \textbf{$q = 1$  $p = 5$}                                                  &  \textbf{$H$= 0,9999}                                                       & Sucesso     \\ \hline
\end{tabular}
\end{table}
...
família de mapas tanh. Fixamos o parâmetro $r=3$ e mostramos as melhores taxas encontradas para as duas codificações. Este gerador passa em todos os testes realizados, portanto é um PRNG de 
acordo com o teste NIST. 
...
Random Excursions       & 0,023545      & 0,98200 & Sucesso \\ \hline
Random Excursions Variant       & 0,122325      & 0,98800 & Sucesso \\ \hline
Cumulative Sums         & 0,767582      & 0,99600 & Sucesso \\ \hline
\end{tabular}
\end{table}

\begin{table}[!h]
\caption{\label{tab-nist2} $P$-$value$ e a razão de testes aprovados para cada teste do NIST, para o mapa 
e-tanh com $r=3$, $R_x=1/7$, com CFT.}
\centering
\begin{tabular}{|l|c|c|c|}
\hline
Teste Estatístico     & $P$-${value}$   & Aprovados   & Resultado          \\ \hline
Frequency               & 0,350485      & 0,99500 & Sucesso \\ \hline
Block Frequency          & 0,035174      & 0,98800 & Sucesso \\ \hline
Runs                    & 0,914463      & 0,99500 & Sucesso \\ \hline
Longest-Run-of-Ones              & 0,534146      & 0,99600 & Sucesso \\ \hline
Binary Matrix Rank                    & 0,739918      & 0,99800 & Sucesso \\ \hline
FFT                     & 0,197981      & 0,98500 & Sucesso \\ \hline
Non-Overlapping Template & 0,911413      & 0,98900 & Sucesso \\ \hline
Overlapping Template    & 0,213309      & 0,98900 & Sucesso \\ \hline
Universal               & 0,122325      & 0,99100 & Sucesso \\ \hline
Linear Complexity        & 0,535129      & 0,98600 & Sucesso \\ \hline
Serial                  & 0,901653      & 0,99800 & Sucesso \\ \hline
Approximate Entropy     & 0,350485      & 0,99800 & Sucesso \\ \hline
Random Excursions       & 0,125695      & 0,98500 & Sucesso \\ \hline
Random Excursions Variant       & 0,325585      & 0,99300 & Sucesso \\ \hline
Cumulative Sums         & 0,534146      & 0,99100 & Sucesso \\ \hline
\end{tabular}
\end{table}

\begin{table}[!h]
\caption{\label{tab-nist3} $P$-$value$ e a razão de testes aprovados para cada teste do NIST, para o mapa 
o-tanh com $r=3$, $R_x=4/4$, com CVT.}
\centering
\begin{tabular}{|l|c|c|c|}
\hline
Teste Estatístico     & $P$-${value}$   & Aprovados   & Resultado          \\ \hline
Frequency               & 0,911413      & 0,98100 & Sucesso \\ \hline
Block Frequency          & 0,534146      & 0,98900 & Sucesso \\ \hline
Runs                    & 0,739918      & 0,99400 & Sucesso \\ \hline
Longest-Run-of-Ones              & 0,215519      & 0,99100 & Sucesso \\ \hline
Binary Matrix Rank                    & 0,750116      & 0,99800 & Sucesso \\ \hline
FFT                     & 0,350485      & 0,98300 & Sucesso \\ \hline
Non-Overlapping Template & 0,528462      & 0,98800 & Sucesso \\ \hline
Overlapping Template    & 0,213309      & 0,98200 & Sucesso \\ \hline
Universal               & 0,122325      & 0,97900 & Sucesso \\ \hline
Linear Complexity        & 0,684381      & 0,99700 & Sucesso \\ \hline
Serial                  & 0,136457      & 0,99600 & Sucesso \\ \hline
Approximate Entropy     & 0,832484      & 0,99800 & Sucesso \\ \hline
Random Excursions       & 0,469832      & 0,98300 & Sucesso \\ \hline
Random Excursions Variant       & 0,658771      & 0,99200 & Sucesso \\ \hline
Cumulative Sums         & 0,657894      & 0,99600 & Sucesso \\ \hline
\end{tabular}
\end{table}

\begin{table}[!h]
\caption{\label{tab-nist4} $P$-$value$ e a razão de testes aprovados para cada teste do NIST, para o mapa 
o-tanh com $r=3$, $R_x=1/5$, com CFT.}
\centering
\begin{tabular}{|l|c|c|c|}
\hline
Teste Estatístico     & $P$-${value}$   & Aprovados   & Resultado          \\ \hline
Frequency               & 0,925478      & 0,99700 & Sucesso \\ \hline
Block Frequency          & 0,569871      & 0,98600 & Sucesso \\ \hline
Runs                    & 0,125478      & 0,98200 & Sucesso \\ \hline
Longest-Run-of-Ones              & 0,289635      & 0,984100 & Sucesso \\ \hline
Binary Matrix Rank                    & 0,515896      & 0,99100 & Sucesso \\ \hline
FFT                     & 0,478956      & 0,98800 & Sucesso \\ \hline
Non-Overlapping Template & 0,157896      & 0,98700 & Sucesso \\ \hline
Overlapping Template    & 0,158963      & 0,98200 & Sucesso \\ \hline
Universal               & 0,089637      & 0,98000 & Sucesso \\ \hline
Linear Complexity        & 0,735961      & 0,99400 & Sucesso \\ \hline
Serial                  & 0,358942      & 0,99100 & Sucesso \\ \hline
Approximate Entropy     & 0,351789      & 0,99300 & Sucesso \\ \hline
Random Excursions       & 0,75489      & 0,99000 & Sucesso \\ \hline
Random Excursions Variant       & 0,656983      & 0,98900 & Sucesso \\ \hline
Cumulative Sums         & 0,485236      & 0,98500 & Sucesso \\ \hline
\end{tabular}
\end{table}

Convém ressaltar que $n^{*}=7$ e $n^{*}=5$ são os valores indicados para o salto, de acordo com a entropia condicional mostrados nas Figuras~\ref{fig:entropiar} e \ref{fig:entropiai}, para a 
quebra da correlação das sequências binárias geradas pelos mapas e-tanh e o-tanh, respectivamente, com $q=1$ e valores de $r$ entre 3 e 5. Portanto, a informação gerada pela entropia 
condicional é importante para a determinação do valor do salto de amostras $p$.

%%%%%%%%%%%%%%%%%%%%%% FIM DA SEÇÃO -  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Geração de um PRNG com pós-processamento usando \\sequências-m}
...
Nesta seção, não empregamos salto de amostras (portanto, adotamos nesta seção $p = 1$) e esta correlação é eliminada por um pós-processamento após a codificação, empregando a sequência gerada 
por um 
LFSR. O foco desta seção é o projeto do comprimento do LFSR. Algumas propriedades destas sequências são descritas na próxima subseção.

O esquema para a geração de um PRNG considerado nesta seção tem três blocos, conforme é mostrado na Figura \ref{fig:22}. Considerando uma codificação de $q$ bits, cada variável $X_n$ é mapeada em 
...
Cada sequência de símbolos na saída do codificador é mapeada na saída da unidade de pós-processamento com o mesmo comprimento, portanto, esta unidade tem taxa unitária. Assim, a taxa total 
do sistema da Figura~\ref{fig:22}, em \textit{bits} por amostra caótica, é $R_x=q$. A sequência $\{Y_k\}$ é a entrada da bateria de teste NIST.

\begin{figure}[t!]
  \begin{pspicture}(-.6,0.5)(6.5,3)
 \psline{->}(4,1.5)(5,1.5)
 \psframe(1,1)(4,2)
 \rput{0}(2.5,1.5){\small Mapa tanh}
 \psframe(5,1)(8,2)
...
\begin{equation}
 p(x) = 1 + c_1 x + c_2 x^{2} + ... + c_{N-1}x^{N-1} + x^N.
\end{equation}
...
possível, $2^N-1$, para uma sequência $\{W_k\}$, o polinômio $p(x)$ deve ser primitivo \cite{crypt}. A sequência gerada por um LFSR com um polinômio primitivo é chamada 
...
enquanto que, para $n>N$, tende a zero devido à repetição dos valores das subsequências de comprimento maior que $N$, como ilustra a Figura \ref{fig:LFSREnt}. 
As subsequências de comprimento $n \leq N$ acontecem com probabilidade aproximadamente igual a $\frac{1}{2^n}$, apresentado uma  estatística local quase ideal de ordem $N$. Esta aproximação 
melhora com o aumento de $N$ e será considerada nas análises a seguir. Esta característica 
...
\subsection{Esquema de geração de um PRNG com pós-processamento}

Para aproveitar as boas propriedades estatísticas das sequências-m propõe-se nesta subseção um método para gerar um PRNG que é uma combinação da técnica de discretização associada à 
codificação de amostras de um mapa caótico, seguido por um pós-processamento com uma sequência-m. Inicialmente, obtém-se uma sequência binária a partir da discretização de um mapa caótico (usando um dos métodos, CFT ou CVT) 
e realiza-se uma soma módulo 2 dela com uma sequência-m gerada por um LFSR. Este método é ilustrado na Figura \ref{fig:bloco2}. Seja $\{Z_k\}$ a sequência na saída de um codificador CFT ou CVT 
e $\{W_k\}$ a sequência-m gerada pelo LFSR, então a $k$-ésima amostra de saída $\{Y_k\}$ é dada por
\begin{equation}
 Y_k = Z_k \oplus W_k.
\end{equation}
Suponha que a sequência $\{Z_k\}$ é formada com $q=1$ (dois níveis de quantização) e é independente de $\{W_k\}$. Suponha ainda que estas sequências satisfazem às seguintes condições:
...
\begin{figure}[h!]
 ...
 \rput{0}(9.75,0.7){\small $\{W_k\}$}
 \end{pspicture}
 \caption{\label{fig:bloco2} Diagrama em blocos do esquema de geração de um PRNG usando sequências-m.}
\end{figure}
...
\begin{itemize}
 \item A sequência $\{W_k\}$ tem estatística local ideal de ordem $N$, isto é, para $n \leq N$:
 \begin{equation}
  \Pr(w_1 ... w_n) = \Pr (W_1 = w_1, ..., W_n = w_n) = \frac{1}{2^n}.
 \end{equation}
 \item Cada variável aleatória $Z_k$ tem distribuição uniforme, $\Pr (Z_k = 0) = \Pr (Z_k = 1) = \frac{1}{2}$, e duas variáveis aleatórias $Z_k$ e $Z_{k+n}$ são estatisticamente independentes 
 para $ n \geq N$, ou seja
 \begin{equation}
  \Pr(z_k z_{k+m}) = \Pr(z_k) \Pr(z_{k+n}) = \frac{1}{4}.
 \end{equation}
\end{itemize}
Prova-se, a seguir, que se as condições (3.5) e (3.6) são satisfeitas, a sequência $\{Y_k\}$ é formada por variáveis aleatórias independentes e identicamente distribuídas com distribuição uniforme. 
Observa-se que a condição (3.6) implica que o valor de $N$ deve ser escolhido de tal forma que seja maior ou igual a memória existente na sequência $\{Z_k\}$ que é devida à memória existente 
na sequência $\{X_k\}$.
...
\begin{eqnarray}
 ...
 &=& \frac{1}{2^n}
\end{eqnarray}
em que (3.8) segue da independência entre $\{Z_k\}$ e $\{W_k\}$ e (3.9) segue de (3.5). Considere agora que $n\geq N$, então
\begin{eqnarray}
 ...
 \!&=& \! (\frac{1}{2})^2 \sum_{w_k,w_{k+n}} \Pr(w_k w_{k+n})
\end{eqnarray}
em que (3.13) e (3.14) seguem de (3.6).
...
$\ell=k-1, ..., k-(N-1)$, segue de (3.10) e para $\ell < k-N$ segue de (3.14). Portanto, para $n > N$
\begin{eqnarray}
 \Pr(y_1 ... y_n) &=& \Pr(y_n | y^{n-1}) \cdot \Pr(y_{n-1} | y^{n-2}) \cdot \cdot \cdot \Pr(y_{N+1} | y^{N}) \cdot \Pr(y^{N}) \\ 
...
\begin{figure}[!t]
 \centering %
 \begin{pspicture}(-.6,-.15)(6.5,3.5) 
 \rput(3,2){\includegraphics[scale=0.65]{Figuras/LFSR1}}
 \rput{0}(3.2,-.05){\small $n$}
...
\begin{figure}[!t]
 \centering %
 \begin{pspicture}(-.6,-.15)(6.5,3.5) 
 \rput(3,2){\includegraphics[scale=0.65]{Figuras/LFSR2}}
 \rput{0}(3.2,-.05){\small $n$}
 \rput{90}(-.5,2.1){\small $H(Y_n /Y^{n-1})$}
 \end{pspicture}
...
nas sequências código que rotulam cada amostra caótica. Propomos, então, um valor estimado $\hat{N}$ do grau do polinômio do LFSR para gerar uma sequência $\{Y_k\}$ com boas 
propriedades de aleatoriedade da seguinte forma:
\begin{equation}
 \hat{N} = n^{*} \cdot q.
\end{equation}
Seja $N_{min}$ o menor valor do grau $N$ do polinômio para que a sequência $\{Y_k\}$ passe no teste NIST. A Tabela ~\ref{tab:LM1} %e ~\ref{tab:LM2} 
compara os valores de $\hat{N}$ e $N_{min}$ para diferentes taxas $R_x = q$ para os mapas e-tanh e o-tanh com $r=3$ e para codificação CFT 
(o caso CVT será tratado no próximo capítulo). Observa-se nesta tabela que $N_{min}$ cresce aproximadamente linear com $q$ sendo bem 
aproximado por $\hat{N}$ (uma discrepância entre $N_{min}$ e $\hat{N}$ de 
no máximo igual a 3 é observada nesta tabela). Este comportamento é observado também na Figura~\ref{fig:Nq} para os mapas e-tanh e o-tanh.
...
\begin{table}[!t]
\centering
\caption{Comparação entre $N_{min}$ e $\hat{N}$ para o mapa e-tanh ($n^{*}=7$) e o-tanh ($n^{*}=5$) com $r=3$ e codificação CFT.}
\label{tab:LM1}
...
Para gerar taxas maiores é preciso um aumento do valor de $N_{min}$, mas, os requerimentos de memória do LFSR cresce 
linearmente com a taxa de \textit{bits}. Uma unidade de pós-processamento com menor requerimento de memória é proposta no próximo capítulo.
%As Figuras 
%\ref{fig:nvsqp} e \ref{fig:nvsqi} mostram este comportamento. 

\begin{figure}[!t]
 \centering %
 \begin{pspicture}(-.6,-.4)(6.5,4) 
 \rput(3,2){\includegraphics[scale=0.7]{Figuras/Nq}}
 \rput{0}(3.15,-.15){\small $q$}
 \rput{90}(-.5,2.1){\small $N_{min}$}
 \end{pspicture}
 \caption{\label{fig:Nq} $N_{min}$ versus $q$ para os mapas e-tanh e o-tanh com codificação CFT.}
\end{figure}
%\begin{figure}[!h]
% \centering %
% \begin{pspicture}(-.6,-.4)(6.5,4) 
