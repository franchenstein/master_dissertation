...
To find the synchronization words, algorithm~\ref{alg:findsynchwords} is used. It receives as parameters both trees and \textit{W}. The lists $\Gamma$ and $\Delta$ are then created as described above. Besides that, the list $\Omega_{syn}$, which will receive the results from the algorithm is initialized as an empty list. Another list $\Theta$ is created to store triples \textit{(t, s, result)}, where \textit{t} is a state from $\mathcal{T}$, \textit{s} is a state from $\mathcal{S}$ and \textit{result} is the result from the statistical test of comparing the morphs of \textit{t} and \textit{s}. Whenever a statistical test needs to be performed, the result is first checked in this list. If the test has been performed in a given pair of states before, it saves time not to do it again. $\Theta$ is initialized empty.
...
If there is a valid state \textit{c}, a new list $\Psi$ is created empty. It is used to store states from $\mathcal{T}$ for which the suffix was already found. This is done in order to save time. If the suffix was found the algorithm will not search for it again. If the label of \textit{c} is shorter than \textit{W}, another list $\Lambda$ is created with all the elements from $\Delta$ whose lengths are less than the length of \textit{c}. They also cannot be present in the $\Psi$ list nor the pair \textit{(c.label, s)} can be present in the list $\Theta$. These restrictions prevent the algorithm from doing redundant work of finding suffixes which were already found or repeating tests for which the results are known.

Each element $\lambda$ of $\Lambda$ is then compared to \textit{c}. First, it is checked if \textit{c.label} is a suffix of $\lambda$. In the affirmative case, $\lambda$ is added to $\Psi$. Then the label of \textit{c} and $\lambda$ are compared via a statistical test.
    ...
    \begin{algorithmic}[1]
      ...
      \Procedure{MainLoop}{}
        ...
        \Else
          ...
          \If {$\textit{l} < \textit{W}$}
            \State $\Lambda \gets \{\textit{s} \in \Delta : length(s) > l\text{, }\textit{s} \notin \Psi \text{, } (\textit{c.label}, \textit{s}) \notin \Theta \}$
            \For{$\text{each } \lambda \in \Lambda$} 
              \If{$\textit{c.label}$ is a suffix of $\lambda$}
                \State $\Psi \gets \Psi\cup\lambda$
                \State $\textit{p} \gets \mathcal{V}(c(label)) = \mathcal{V}(h)$
                \State $\Theta \gets \Theta\cup(c.label, \lambda, p)$
                \If{$\textit{p} = False$}
