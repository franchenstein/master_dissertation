...
que $N_{min}$ cresce aproximadamente linearmente com $q$ sendo bem aproximado por $\hat{N}$ (uma discrepância entre $N_{min}$ e $\hat{N}$ de no máximo igual a 3 é observada nesta tabela) quando 
utilizamos codificação CFT.

Quando utiliza-se a codificação CVT este comportamento afasta-se do valor esperado, obtendo valores de $N_{min}$ menores que $\hat{N}$ com uma discrepância grande em comparação com a codificação 
CFT.
...
Pode-se ver que com o aumento do valor de $N$ pode-se gerar taxas maiores, que no entanto, os requerimentos de memória do LFSR cresce linearmente com a taxa de \textit{bits}, com codificação CFT. 
As Figuras \ref{fig:nvsqp} e \ref{fig:nvsqi} mostram este comportamento para dicha codificação. Uma unidade de pós-processamento com menor requerimento de memória será proposta no próximo capítulo, 
como o estudo do comportamento da codificação CVT.
